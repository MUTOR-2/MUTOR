...
authors: 
 - Georg Hajdu
 - John MacCallum
topics: [Scale, harmony, microtonality]
test_questions:
...
Listening example 1.  The first minute of Capriccio (FbWV 506) by Johann Jakob Froberger played on an organ in a tuning system other than equal temperament.
...
</html>Listening example 2.  Froberger's Capriccio played in equal temperament.
...
# Period Tunings
A tuning where all thirds and fifths are just cannot exist mathematically, therefore all tunings such as the following are compromises:
...
53TET is derived from the stacking of 53 fifths. 53 fifths correspond to 31 octaves in a very good approximation. 

Tuning systems in which a __diatonic__ pitch set exists are also interesting, such as at 17TET, 19TET (1/3 comma mean-tone tuning), 22TET, 31TET (1/4 comma mean-tone tuning) and 41TET. 

These systems obey the following rule: 5 * m * s + 2 * n * s = 1200, where s is the step size of the system, and m and n are integers to be multiplied with s by to yield a diatoic whole tone or a semitone, resp. 
