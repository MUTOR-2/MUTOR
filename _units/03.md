...
# Introduction
In the previous unit we introduced various physical properties of the sound stimulus. We saw that frequency, ampltude, phase, temporal envelopes, and spectral envelopes are all useful descriptors of sounds, and that different visual representations may contain useful information about these sound properties.
...
{% include img-figure url="/MUTOR/assets/images/unit3_lakescene.jpg" description="Lake Scene" %}
...
For example, here is one simple question that a psychoacoustician might ask: how loud does a tone have to be before it is heard? This question can be addressed using a simple signal detection test, where the listener is presented with many trials. Each trial may contain either a tone in a noisy background (signal-plus-noise trial), or background noise only (noise trial). After hearing each trial, the listener must decide whether they heard a noise trial or a signal-plus-noise trial. This test, known as the tone-detection-in-noise task, is one of the simplest of psychoacoustical methods. The accuracy of the listener's performance, as measured either by the percent of correct responses (PC, or Percent Correct) or by the listener's sensitivity (d' score, or discriminability as determined by SignalDetectionTheory methods), gives the psychoacoustician an idea of the signal-to-noise ratio necessary for a listener to reliably detect the tone. As you can see from the discussion in this unit on LoudnessPerception, the answer to such a simple question can be a complex one, and may depend on other variables such as the frequency and location of the signal. 
...
Simple signal detection theory assumes two Gaussian distributions of signal trials and noise trials respectively. Given some amount of evidence of the signal, plotted on the x axis (or "internal response" as shown in the graph), the observer must decide on the probability that the evidence points to a signal trial or a noise trial. The observer then adopts a criterion (shown as the thick line labeld "criterion response"). Any trial for which the internal response exceeds the criterion is categorized as "signal", and any trial for which the internal response is below criterion is categorized as "noise". 

{% include img-figure url="/MUTOR/assets/images/unit3_sdt1.gif" description="Signal Detection Theory illustration" %}
...
d' is a measure of sensitivity, given by the distance between the means of the two distributions. 
d' = z(hit) - z(false alarm),
where z is the deviation of the criterion from the mean, given in standard units (z-scores). 
Since d' is a measurement of how far apart the two distributions are, a higher d' means that signal trials and noise trials are very clearly separated; thus, higher d' signifies better performance. 

{% include img-figure url="/MUTOR/assets/images/unit3_sdt2.gif" description="SDT Criterion" %}


(image source: http://www.cns.nyu.edu/~david/sdt/sdt.html)

Criterion adoption (i.e. how liberal you are at labelling a trial as "signal") does not affect sensitivity as measured by d'.

{% include img-figure url="/MUTOR/assets/images/unit3_sdt3.gif" description="d prime illustration" %}

(image source: http://www.cns.nyu.edu/~david/sdt/sdt.html)

d' scores - the higher the d', the more discriminable the two types of trials are; thus, the better the performance.
...
The intensity of a sound is proportional to the square of the amplitude and is measured in watts per meter^2 
...
\begin{equation}
dB=10log_1_0(\frac{I}{I_0})
\label{eq:dB-intensity}
\end{equation}

for intensity. As we an see from these equations, the decibel is not an absolute measure, but rather a comparison against a reference. In acoustics, the measure dB(SPL—Sound Pressure Level) is often used which indicates that the reference is 10^{-9} of atmospheric pressure, 2x10^{-5}N/m^2 at 1000Hz, or 10^{-12} watts for power. Although this may seem complicated, the decibel is really a convenient measure of loudness since our ears work on something closer to a logarithmic scale. See figure 1 for a common sounds and their dB levels.
...
Krakatoa explosion at 100 miles (160 km) in air [1]
...
Figure 1: Some common sounds and their volumes represented in dB (http://en.wikipedia.org/wiki/Decibel).
...
{% include img-figure url="/MUTOR/assets/images/unit3_loudness1-screenshot.png" description="loudness1 demo" %}
...
{% include img-figure url=/MUTOR/assets/images/"unit3_Fletcher-Munson.gif" description="fm curve" %}
...
{% include img-figure url="/MUTOR/assets/images/unit3_fm-curve.png" description="fm curve2" %}
...
{% include img-figure url="/MUTOR/assets/images/unit3_stephens_plaw_formula.png" description="stephens power law" %}
...
{% include img-figure url="/MUTOR/assets/images/unit3_loudness2-screenshot.png" description="loudness2" %}
...
{% include img-figure url="/MUTOR/assets/images/unit3_loudness3-screenshot.png" description="loudness3" %}
...
{% include img-figure url="/MUTOR/assets/images/unit3_tuba-cresc.png" description="tuba spectrum cresc" %}
Figure 3: A tuba playing A3 going from moderately loud to soft to loud.
 
![Tuba Crescendo](/MUTOR/assets/images/tuba-cresc.aif?controlsList=nodownload-nofullscreen-noremoteplayback)
Audio example 1: The tuba from figure 3.
...
{% include img-figure url="/MUTOR/assets/images/unit3_loudness4-screenshot.png" description="loudness4" %}
...
![Example 1](/MUTOR/assets/images/unit3_larry1.wav?controlsList=nodownload-nofullscreen-noremoteplayback)
![Example 2](/MUTOR/assets/images/unit3_Psycho1.wav?controlsList=nodownload-nofullscreen-noremoteplayback)
...
Reverberation (or reverb) is the result of sounds reflecting off of the surfaces of an enclosed space. Imagine that we make a loud noise (by firing a starter's pistol or poping a balloon) in a cathedral or a hall. The first sound that a listener would hear is called the direct sound (figures 1-3).

{% include img-figure url="/MUTOR/assets/images/unit3_roomAcoustics-nosound.png" description="Room Acoustics No Sound" %}
Figure 1: A listener in a room in which a loud short sound is made.

{% include img-figure url="/MUTOR/assets/images/unit3_roomAcoustics-direct.png" description="Room Acoustics Direct Sound" %}
Figure 2: The first sound to hit the listener is called the direct sound.

{% include img-figure url="/MUTOR/assets/images/unit3_reverb-direct-plot.png" description="Reverb Direct Plot" %}
Figure 3: The direct sound from figure 2.

The direct sound is followed by a number of early reflections that arive at the listener after some delay and loss of energy (figures 4-5). The length of the delay and the amount of energy lost are dependant on the size of the room and the type of material used to make the surfaces (the walls, floor, and ceiling).

{% include img-figure url="/MUTOR/assets/images/unit3_roomAcoustics-1stRef.png" description="Room Acoustics First Reflections" %}
Figure 4: The early reflections arrive at the listener after a short delay and with some loss of energy.

{% include img-figure url="/MUTOR/assets/images/unit3_reverb-direct-early-plot.png" description="Reverb Direct Early Plot" %}
Figure 5: The early reflections from figure 4 plotted against time.

As these early reflections continue to bounce off the surfaces they begin to pile up and overlap creating reverb (figures 6-7).

{% include img-figure url="/MUTOR/assets/images/unit3_roomAcoustics-reverb.png" description="Room Acoustics Reverb" %}
Figure 6: Reverb is as the original sound continues to bounce around the room, we perceive reverb.

{% include img-figure url="/MUTOR/assets/images/unit3_reverb-plot.png" description="Reverb Plot" %}
Figure 7: The reverb in a room tends to decay exponentially.
...
where P is the initial power of the signal, r is the distance from the source in meters, and I is the intensity in W/m^2. Notice that the denominator in the above equation is the area of a sphere due to the fact that sound radiates out in all directions in a spherical pattern. As sound continues to propagate away from the source, it diffuses over a larger and larger area (figure 8). Imagine that you are standing one meter away from a sound source and the intensity of the sound when it reaches you is 100 dB. If you move 10 meters away from the source, the sound will be spread over 100 times the area and the resulting intensity will be 1/100 of the original or 80 dB.

unit3_inverse-square-diagram.png
Figure 8: Sound propagating away from a source diffuses over a larger and larger area.

The later reflections also depend on the absorptive quality of the surfaces they bounce off of and can be calculated by

\begin{equation}
Ir={missing equation}
\label{eq:missing-equation}
\end{equation}
...
where RT_60 is the reverb time, V is the volume of the enclosure, A is the sum of the absorption coefficients for each surface, and C is a constant that takes on 0.161 for calculations in meters and 0.049 for feet.

Sound examples 1-3, and figures 9-11 are impulse responses of three different halls and their sonograms. Notice that the reverb of each hall decays in a different way and that different frequency regions decay quicker than others but that in general the low frequencies remain active longer than the high frequencies—this is a typical property of the reverb of most enclosed spaces. Also, note that the early reflections can be heard clearly in the second and third examples.


Impulse 1 unit3_impulse1.wav
Impulse 2 unit3_impulse2.wav
Impulse 3 unit3_impulse3.wav

unit3_impulse-specgram-decayenv.png
unit3_impulse2-specgram-decayenv.png
unit3_impulse3-specgram-decayenv.png

Figures 9-11: The reverb decays of three different rooms.
...
Children's song "Frère Jacques" unit3_frerejacques.mid

The opening measures of Mahler's Symphony No. 1 in D, "Titan" unit3_mahler1.mp3.
...
unit3_topdownillusion.jpg Top Down
...
unit3_topdownillusion2.jpg Perceptual illusion corrected
...
unit3_happybday.jpg happy birthday 

A line from the Happy Birthday song, illustrating the principle of gap fill. More on these top-down expectations for melodic processes will be discussed in Unit 7.
...
unit3_precedence.gif Precedence Effect
...
see also: FranssenEffect, CliftonEffect
...
unit3_preceng.jpg  Engineering Applications
...
Clifton Effect Demonstration (requires stereo headphones) 
(source: http://www.parmly.luc.edu/parmly/clifton.html)
...
boltz.jpg Boltz soundtrack study
(image source & reference: http://caliber.ucpress.net/doi/pdfplus/10.1525/mp.2001.18.4.427)
...
<html>
<object width="425" height="350"><embed src="babyvent.mpg" width="425" height="350" autoplay="false"></embed></object>
        </html>
  
(video source: http://www.paulzerdin.com) 
(see also http://video.google.com/videoplay?docid=8126344216754570223&q=ventriloquist+is%3Afree+duration%3Ashort&pr=goog-sl)
...
<html>
<embed name="McGurk_large.mov" SRC="McGurk_large.mov" width="400" height="320" autoplay="false"></html>
 
(video source: http://www.media.uio.no/personer/arntm/McGurk_english.html)
...
In the previous unit, we discussed sound largely in the abstract, whereas here we have seen that the environment and our auditory system both play a large role in our perception and understanding of the sonic world around us. It is not enough to say that a signal has an amplitude of 0.5—as we have seen, this will mean different things depending on the frequency. From the bottom-up, we can say that the signal has an amplitude of 0.5 and that the loudness will vary depending on the frequency due to the fact that our auditory system is more sensitive at certain frequencies than others; from the top-down, our knowledge of the origin of the sound source (what made it, where it comes from, etc.) may influence the way in which we hear the signal. This top-down processing can lead to interesting auditory illusions such as the McGurk effect, the Franssen effect, and the Clifton effect. Finally, the acoustic environment and how much reverberation that environment adds to a sound dramatically alters the way in which perceive sounds.
